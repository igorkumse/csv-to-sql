        ...
        with open(file_path, 'r', encoding='utf-8') as rf, \
            ...
            for line in rf:
                ...
                line_insert = (
                    ...
                    f"{fin_inst_in}, {org_enota_in}, {aplikacija_in}, {kom_stevilka_in}, "
                    f"{apl_domicil_in}, {status_in}, {fin_inst_out}, {org_enota_out}, "
                    f"{aplikacija_out}, {kom_stevilka_out}, {apl_domicil_out}, {status_out}, current timestamp);"
                )
                ...
                line_update = (
                    ...
                    f"KOM_STEVILKA_OUT = {kom_stevilka_out}, APL_DOMICIL_OUT = {apl_domicil_out}, "
                    f"STATUS_OUT = {status_out} "
                    f"WHERE FIN_INST_IN = {fin_inst_in} AND ORG_ENOTA_IN = {org_enota_in} "
                    f"AND APLIKACIJA_IN = {aplikacija_in} AND KOM_STEVILKA_IN = {kom_stevilka_in};"
                )
    ...
    while True:
        try:
            for file in INPUT_DIR.glob("*.csv"):
                process_file(file)
            time.sleep(30)  # čaka 30 sekund pred naslednjim preverjanjem
        except Exception as e:
